...
int main()
{
    __m256 m_load1, m_load2, m_store;
	int cnt = 9;
    float *p_f1 = (float*)aligned_malloc(sizeof(float) * cnt, 64);
	float *p_f2 = (float*)aligned_malloc(sizeof(float) * cnt, 64);
	float *p_f3 = (float*)aligned_malloc(sizeof(float) * cnt, 64);
	for(int i = 0; i < cnt; i++)
	{
		p_f1[i] = 0.1 * i;
		p_f2[i] = 0.4 * i;
	...
	{
		m_load1 = _mm256_load_ps(p_f1+i * 8);
		m_load2 = _mm256_load_ps(p_f2+i * 8);
		m_store = _mm256_add_ps(m_load1, m_load2);
		_mm256_store_ps(p_f3+i*8, m_store);
